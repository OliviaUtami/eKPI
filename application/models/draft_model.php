...
class draft_model extends CI_Model {
  public function get_draft(){
    $sql = "SELECT 
              draft_id, draft_name, status, DATE_FORMAT(created_at, '%d/%m/%Y') created_on, created_by, 
              DATE_FORMAT(approved_at, '%d/%m/%Y') approved_at, approved_by
            FROM `drafts` 
            ORDER BY draft_id DESC;";
    return $this->db->query($sql)->result();
  }

  public function get_draft_by_id($period_id){
    $sql = "SELECT 
  ...
  public function add_period($period_from, $period_to, $status){
    $message = ""; $ok = 1;
    ...
    if(1==0){
      ...
      $this->db->query($sql, array($period_from, $period_to, $status, 'IMPORT'));
