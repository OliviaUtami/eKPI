...
class organization_model extends CI_Model {
  public function get_organizations(){
    return $this->db->query("select * from organizations where is_active = ? order by hierarchy asc", array(1))->result();
  }
  public function add_org($name, $hierarchy){
    $message = ""; $ok = 1;
    $exist = $this->db->query("SELECT COUNT(1) count FROM organizations WHERE hierarchy = ? and is_active = 1",array($hierarchy))->row();
    ...
    if((int) $exist->count > 0){
      ...
      $sql = "INSERT INTO organizations 
            ...
            VALUES (?, ?, ?, ?)";
      $this->db->query($sql, array($name, $hierarchy, 1, 'IMPORT'));
  ...
  public function edit_org($id, $name, $hierarchy){
    $message = ""; $ok = 1;
    $exist = $this->db->query("SELECT COUNT(1) count FROM organizations WHERE hierarchy = ? and is_active = 1 and org_id <> ?",array($hierarchy, $id))->row();
