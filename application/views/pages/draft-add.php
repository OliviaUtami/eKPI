...
<div class="modal" id="modal-tambah-tujuan" tabindex="-1" role="dialog">
  <div class="modal-dialog" role="document">
    <div class="modal-content">
      ...
      <div class="modal-footer">
        <button type="button" class="btn btn-primary" onclick="saveTujuan()">Simpan</button>
...
<div class="modal" id="modal-tambah-target" tabindex="-1" role="dialog">
  <div class="modal-dialog" role="document">
    <div class="modal-content">
      <div class="modal-header">
        <h5 class="modal-title">Tambah Sasaran</h5>
        <button type="button" class="close" data-dismiss="modal" aria-label="Close">
          <span aria-hidden="true">&times;</span>
        </button>
      </div>
      ...
      <div class="modal-footer">
        <button type="button" class="btn btn-primary" onclick="saveTarget()">Simpan</button>
        <button type="button" class="btn btn-secondary" data-dismiss="modal">Tutup</button>
      </div>
    </div>
  </div>
</div>
<div class="modal" id="modal-tambah-program" tabindex="-1" role="dialog">
  <div class="modal-dialog" role="document">
    <div class="modal-content">
      <div class="modal-header">
      ...
      <div class="modal-body">
        <input id="inpTempIDMisi" class="form-control" type="hidden" autocomplete="off"/>
        <input id="inpTempIDTujuan" class="form-control" type="hidden" autocomplete="off"/>
        <input id="inpTempIDTarget" class="form-control" type="hidden" autocomplete="off"/>
        <input id="inpTempID" class="form-control" type="hidden" autocomplete="off"/>
        <textarea id="inpProgram" class="form-control" autocomplete="off"></textarea>
      </div>
      <div class="modal-footer">
        <button type="button" class="btn btn-primary" onclick="saveProgram()">Simpan</button>
        <button type="button" class="btn btn-secondary" data-dismiss="modal">Tutup</button>
      </div>
    </div>
  </div>
...
<script>
  ...
  $(document).on("click","#btn-save",function(){
    ...
    for (var i=0; i<misi.length; i++) {
      ...
      if(arrtujuan.length==0){
        alert("Misi harus memiliki minimal 1 tujuan");
        return;
      }
      for (var j=0; j<arrtujuan.length; j++){
        ...
        if(arrtarget.length==0){
          alert("Tujuan harus memiliki minimal satu sasaran");
          return;
        }
        for (var k=0; k<arrtarget.length; k++){
          ...
          if(arrprogram.length==0){
            alert("Sasaran harus memiliki minimal 1 program");
            return;
          }
        }
      }
  ...
  function deleteMisi(tempid){
    if(confirm("Konfirmasi hapus misi dalam draft ini?")){
    ...
    for(var i=0;i<misi.length; i++){
      ...
      for(var j=0;j<misi[i].tujuan.length;j++){
        ...
        for(var k=0;k<tujuan.target.length;k++){
          ...
          if(k==0){
            html_target = "<table class='table-target tujuan-"+tujuan.tempid+" table table-striped table-bordered' cellspacing='0' width='100%'>" +
                            "<thead>" +
                                "<tr>"+
                                    "<th style='width:30px'>No</th>" +
                                    "<th>Sasaran</th>" +
