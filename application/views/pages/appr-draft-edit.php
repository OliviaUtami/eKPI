                ...
                <div class="card-body">
                  <div class="row">
                    <div class="form-group col-md-5">
                        ...
                        <input type="text" class="form-control" id="name" name="name" autocomplete="off" value="<?php echo $draft_data->nama; ?>" <?php if($draft_data->status!=="Draft"){ echo "disabled"; } ?> required>
                  ...
                  <div class="row">
                    <div class="form-group col-md-12">
                      <?php if($draft_data->status=='Draft'){ ?>
...
<script>
  ...
  <?php
    echo ("misi = ".json_encode($draft_data->details).";");
  ?>
  //populate
  ...
  $(document).on("click",".btn-process",function(){
    if(confirm(($(this).data("status")=="reject"?"Tolak":"Setujui")+" draft KPI ini ?")){
      if($("#name").val().trim()==""){
      ...
      $.ajax({
          ...
          success: function(data) {
            if(data.ok==1){
              window.location.replace("/eKPI/draft-approval");
            }else{
              alert(data.msg);
            }
          },
    ...
    for(var i=0;i<misi.length; i++){
      ...
      for(var j=0;j<misi[i].tujuan.length;j++){
        ...
        for(var k=0;k<tujuan.target.length;k++){
          ...
          html_target += "<tr class='target-"+target.tempid+"'>" +
                        ...
                        "<tr class='target-"+target.tempid+" tr-program' style='"+(html_program==""?"display:none":"")+"'>" +
                          "<td></td><td colspan='1' style='padding:10px;'>" + html_program + "</td>" +
                        "</tr>";
        ...
        html_tujuan += "<tr class='tujuan-"+tujuan.tempid+"'>" +
                        ...
                        "<tr class='tujuan-"+tujuan.tempid+" tr-target' style='"+(html_target==""?"display:none":"")+"'>" +
                          "<td></td><td colspan='1' style='padding:10px;'>" + html_target + "</td>" +
                        "</tr>";
      ...
      html_misi +=   "" +
                  ...
                  "<tr class='misi-"+misi[i].tempid+" tr-tujuan' style='"+(html_tujuan==""?"display:none":"")+"'>" +
                    "<td></td><td colspan='1' style='padding:10px;'>" + html_tujuan + "</td>" +
                  "</tr>" +
                "";
    }
